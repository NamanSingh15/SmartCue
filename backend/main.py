...
try:
    ...
    from backend import github_mcp, postgres_mcp, calendar_mcp, ai_assistant_mcp

# Feature flags to skip specific MCPs (set SKIP_*="1" to disable)
SKIP_GITHUB = os.getenv("SKIP_GITHUB") == "1"
SKIP_POSTGRES = os.getenv("SKIP_POSTGRES") == "1"
SKIP_CALENDAR = os.getenv("SKIP_CALENDAR") == "1"
...
async def startup_event():
    ...
    if not SKIP_GITHUB:
        ...
        if not github_server:
            logger.error("Failed to start GitHub MCP server")
            # Do not return; allow API to run even if GitHub server failed
    else:
        logger.warning("Skipping GitHub MCP startup (SKIP_GITHUB=1)")
    ...
    if not SKIP_POSTGRES:
        ...
        logger.warning("Skipping Postgres MCP startup (SKIP_POSTGRES=1)")
    ...
    if not SKIP_CALENDAR:
        ...
        logger.warning("Skipping Calendar MCP startup (SKIP_CALENDAR=1)")
    ...
    if not SKIP_AI:
        ...
        logger.warning("Skipping AI Assistant MCP startup (SKIP_AI=1)")
...
async def create_task(task: TaskCreate):
    try:
        ...
        if SKIP_GITHUB:
            raise HTTPException(status_code=503, detail="GitHub is disabled (SKIP_GITHUB=1)")
        ...
        # If Postgres is disabled, return a lightweight response without DB persistence
        if SKIP_POSTGRES:
            logger.warning("Postgres disabled; returning response without DB insert (SKIP_POSTGRES=1)")
            return {
                "task_id": -1,
                "task_name": task.task_name,
                "github_issue_url": issue["issue_url"],
        ...
        if not SKIP_CALENDAR and all([task.meeting_description, task.assignee_for_meeting, meeting_datetime]):
            await calendar_mcp.update_meeting(
                task_id=db_task["task_id"],
                meeting_description=task.meeting_description,
                assignee_for_meeting=task.assignee_for_meeting,
...
async def get_tasks(
    ...
    try:
        if SKIP_POSTGRES:
            logger.warning("Postgres disabled; returning empty task list (SKIP_POSTGRES=1)")
            return {"tasks": []}
        if status and status not in ["open", "notplanned", "completed"]:
            raise HTTPException(
                status_code=400,
...
async def handle_webhook(request: Request):
    try:
        if SKIP_POSTGRES:
            logger.warning("Postgres disabled; ignoring webhook DB update (SKIP_POSTGRES=1)")
            return {"status": "success"}
        payload = await request.json()
        action = payload.get("action", "")
        issue = payload.get("issue", {})
...
async def get_meetings(start_date: Optional[str] = None, end_date: Optional[str] = None):
    try:
        if SKIP_POSTGRES or SKIP_CALENDAR:
            logger.warning("Calendar or Postgres disabled; returning empty meetings (SKIP flags)")
            return {"meetings": []}
        meetings = await calendar_mcp.get_meetings(start_date=start_date, end_date=end_date)
        return {"meetings": meetings}
    except Exception as e:
...
async def ai_assistant(request: AIAssistantRequest):
    ...
    try:
        if SKIP_AI:
            raise HTTPException(status_code=503, detail="AI Assistant is disabled (SKIP_AI=1)")
        ...
        # If Postgres is disabled, return AI result without DB insert
        if SKIP_POSTGRES:
            logger.warning("Postgres disabled; returning AI result without DB insert (SKIP_POSTGRES=1)")
            return {
                "status": "complete",
                "task": {
                    "task_id": -1,
